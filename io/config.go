...
func (jsc *JSONServerConfig) LoadFromFile(file string) error {
    ...
    if jsc.MerkleDepth < sync.MerkleMinDepth || jsc.MerkleDepth > sync.MerkleMaxDepth {
        return errors.New(fmt.Sprintf("Invalid merkle depth specified. Valid ranges are from %d to %d inclusive", sync.MerkleMinDepth, sync.MerkleMaxDepth))
    }
